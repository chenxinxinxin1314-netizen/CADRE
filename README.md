...
Visual recognition systems deployed in evolving environments must accommodate new categories throughout their service life. Repeated retraining on the complete data history is often infeasible, while privacy and data governance constraints may preclude retaining earlier images. Exemplar-free class-incremental learning addresses this setting, yet supervision confined to new-class data introduces an inherent imbalance that shifts old-class representations and biases predictions toward recently introduced classes. CADRE addresses these failures at the stages where they arise by constraining feature drift during encoder adaptation and correcting classifier bias after the representation is fixed. During representation learning, taskwise response distillation is weighted by current teacher--student disagreement, while instance and relational constraints limit changes in individual representations and pairwise feature geometry. Once the encoder is frozen, each class is represented by a mixture of Gaussian sub-prototypes, from which pseudo-features are sampled to recalibrate the joint classifier. The preserved geometry maintains the relevance of statistics estimated at earlier stages for subsequent calibration. Only model parameters and compact class-conditional statistics are carried forward, with neither images nor individual historical features retained. Across three image-classification benchmarks and two task granularities, CADRE improves average incremental accuracy over the strongest competing method in each setting by 5.1--24.5 percentage points and obtains the highest final accuracy and lowest forgetting in all six settings.
