...
class SafeTrivialAugmentWide(transforms.TrivialAugmentWide):
    """Keep training alive when a PIL augmentation returns a corrupted scalar."""
    ...
    def forward(self, img):
        try:
            ...
            if self.fallback_count <= 5:
                print(
                    f"[augmentation fallback {self.fallback_count}] "
...
class SafeTransformPipeline:
    """Fall back to deterministic preprocessing if a training transform fails."""

    def __init__(self, transform, fallback):
    ...
    def __call__(self, img):
        ...
        try:
            ...
            if self.fallback_count <= 5:
                print(
                    f"[transform pipeline fallback {self.fallback_count}] "
                    f"{type(exc).__name__}: {exc}"
                )
...
def get_loaders(datasets, num_tasks, nc_first_task, batch_size, num_workers, pin_memory, validation=.1,
    ...
    for idx_dataset, cur_dataset in enumerate(datasets, 0):
        ...
        trn_transform, tst_transform = get_transforms(resize=dc['resize'],
                                                      test_resize=dc.get('test_resize', 224),
                                                      pad=dc['pad'],
                                                      crop=dc['crop'],
                                                      flip=dc['flip'],
...
def get_transforms(resize, test_resize, pad, crop, flip, normalize, extend_channel, extra_aug="", ds_name=""):
    ...
    elif "tiny_imagenet" in ds_name.lower():
        trn_transform_list = [
    ...
    trn_transform = transforms.Compose(trn_transform_list)
    tst_transform = transforms.Compose(tst_transform_list)
    if "cifar" in ds_name.lower():
        trn_transform = SafeTransformPipeline(trn_transform, tst_transform)
...
def _ensure_imagenet_subset_prepared(path):
    ...
    # Reuse existing index files when the dataset has already been prepared.
    if os.path.exists(f"{path}/train.txt") and os.path.exists(f"{path}/test.txt"):
        print(f"train.txt/test.txt already exist, skipping.")
        return

    print(f'Generating train/test splits for: {path}')

    # Build a deterministic class mapping from sorted directory names.
    train_dir = f"{path}/train"
    classes = sorted(os.listdir(train_dir))
    clsss2idx = {c: i for i, c in enumerate(classes)}
    ...
    prepare_split('train', 'train.txt')
    prepare_split('val',   'test.txt')
    print(f"Done. {len(classes)} classes indexed.")
